...
function checkLocalStorage() {
    if (!localStorage.length) {
        for (let i = 1; i <= 216; i++) {
            localStorage.setItem(`SWSH10 - ${i} - ${cardNames[i]} - Standard`, 0)
            if (!noSecondSetList.includes(i)) {
                localStorage.setItem(`SWSH10 - ${i} - ${cardNames[i]} - Parallel`, 0)
            }
        }
    }
}
...
function drawPage() {
    for (let i = 1; i <= 216; i++) {
        ...
        add_a.onclick=function() {
            j = localStorage.getItem(`SWSH10 - ${i} - ${cardNames[i]} - Standard`)
            localStorage.setItem(`SWSH10 - ${i} - ${cardNames[i]} - Standard`, parseInt(j) + 1)
            document.getElementById(`SWSH10 - ${i} - ${cardNames[i]} - Standard`).innerHTML = localStorage.getItem(`SWSH10 - ${i} - ${cardNames[i]} - Standard`)
        }
        ...
        quantity_a.id=`SWSH10 - ${i} - ${cardNames[i]} - Standard`
        quantity_a.innerHTML=localStorage.getItem(`SWSH10 - ${i} - ${cardNames[i]} - Standard`)
        ...
        if (!noSecondSetList.includes(i)) {
            ...
            add_b.onclick=function() {
                j = localStorage.getItem(`SWSH10 - ${i} - ${cardNames[i]} - Parallel`)
                localStorage.setItem(`SWSH10 - ${i} - ${cardNames[i]} - Parallel`, parseInt(j) + 1)
                document.getElementById(`SWSH10 - ${i} - ${cardNames[i]} - Parallel`).innerHTML = localStorage.getItem(`SWSH10 - ${i} - ${cardNames[i]} - Parallel`)
            }
            ...
            quantity_b.id=`SWSH10 - ${i} - ${cardNames[i]} - Parallel`
            quantity_b.innerHTML=localStorage.getItem(`SWSH10 - ${i} - ${cardNames[i]} - Parallel`)
        }
